...
*                         changed the constructors of std::vector< std::vector<double> > around line 199 to match the new C++,
*                         started using sscanf_s in line 322 and strcpy_s in lines 1620, 1640, 1657, 1766, 1785, 1799, 1912, 2015, 2117, 2226, and 2254 when compiling with MSVS,
*   changes :
...
namespace astIOD {
	...
	*
	...
	)
	{
		...
		for (ll = 1; ll <= 3; ll++)
		{
			...
			ast2Body::rv2coe(r2, v2, p, a, ecc, incl, omega, argp, nu, m, u, l, argper);
	...
	*
	...
	)
	{
		...
		mu = 3.986004418e5;
		#ifdef _MSC_VER
			strcpy_s(error, 12, "ok"); // Use a buffer limit here not to corrupt memory, error is 12 chars long anyway.
		#else
			strcpy(error, "ok");
		#endif
		theta = 0.0;
...
#ifdef _MSC_VER
			strcpy_s(error, 12, "not coplanar");
#else
...
#ifdef _MSC_VER
			strcpy_s(error, 12, "angle > 1ø");
#else
			strcpy(error, 12, "angle > 1ø");
#endif
		}
	...
	*
	...
	)
	{
		...
		//  rp    = sqrt(astMath::mag(ro)*astMath::mag(r)) * (power(cos(dnu * 0.25), 2) + tan2w);
	...
	*
	...
	)
	{
		...
		if (fabs(vara) > small)
		{
			...
			{
#ifdef _MSC_VER
				strcpy_s(error, 12, "gnotconv");
#else
				strcpy(error, "gnotconv");
#endif
				if (ynegktr >= 10){
#ifdef _MSC_VER
					strcpy_s(error, 12, "y negative");
#else
