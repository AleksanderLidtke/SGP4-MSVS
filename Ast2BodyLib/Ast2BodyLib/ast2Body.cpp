...
namespace ast2Body {
	...
	* --------------------------------------------------------------------------- */
	 ...
	 void rv2coe
		 ...
		 )
		 {
		   ...
		   int i;
		   char typeorbit[3];
	//	   astMathclass^ astMathCl = gcnew astMath;
		 ...
		 if ( magh > small )
		   {
			 ...
			 strcpy_s(typeorbit,"ei");
			 if ( ecc < small )
			   {
				 // ----------------  circular equatorial ---------------
				 if  ((incl<small) | (fabs(incl-Pi)<small))
					 strcpy_s(typeorbit,"ce");
				   else
					 // --------------  circular inclined ---------------
					 strcpy_s(typeorbit,"ci");
			   }
			   else
			   {
				 // - elliptical, parabolic, hyperbolic equatorial --
				 if  ((incl<small) | (fabs(incl-Pi)<small))
					 strcpy_s(typeorbit,"ee");
			   }
	...
	*
	...
	//                    dtsec = rem( dtseco,period );
						mulrev = (int) floor(dtseco/period);
