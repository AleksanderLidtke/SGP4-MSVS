...
void tod_gcrf
     ...
     )
     {
       std::vector< std::vector<double> > prec, nut, tempmat, nutp,
              precp;
       double psia, wa, epsa, chia, deltapsi, deltaeps, trueeps, meaneps,
              omega, omgxv[3], tempvec1[3], tempvec[3];
...
void teme_j2k
     (
      ...
      {
        std::vector< std::vector<double> >  prec, nutteme, temp[3][3], tempmat,
               nuttemep, precp;
		...
		coordFK5::precess(ttt, e80, psia, wa, epsa, chia, prec);
		coordFK5::truemean(ttt, order, eqeterms, optteme, iau80rec, nutteme);

        if (direct == eTo)
          {
            astMath::matmult( prec, nutteme, tempmat, 3, 3, 3 );
            astMath::matvecmult( tempmat, rteme, rj2k);
            astMath::matvecmult( tempmat, vteme, vj2k);
            astMath::matvecmult( tempmat, ateme, aj2k);
          }
          else
          {
            astMath::mattrans(nutteme, nuttemep, 3, 3 );
            astMath::mattrans(prec, precp, 3, 3 );

            astMath::matmult( nuttemep, precp, tempmat, 3, 3, 3 );
            astMath::matvecmult(tempmat, rj2k, rteme);
            astMath::matvecmult(tempmat, vj2k, vteme);
            astMath::matvecmult(tempmat, aj2k, ateme);
          }
      }  // procedure teme_j2k
...
void teme_pef
     (
     ...
     )
      {
		...
		//std::vector<double> tm(3, std::vector<double>(3));
		//std::vector<double> tmp(3, std::vector<double>(3));
...
#ifdef _MSC_VER
	   errno_t nutFileErrorCode = fopen_s(&infile, "nut80.dat", "r");
#else
...
void polarm
     ...
     )
     {
       ...
       for (std::vector< std::vector<double> >::iterator it=pm.begin(); it != pm.end();++it)
            it->resize(3);
       double convrt, cosxp, cosyp, sinxp, sinyp, sp, cossp, sinsp, meaneps;
