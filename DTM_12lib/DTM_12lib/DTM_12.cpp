 ...
 * 
 *     current :
 *                9 aug 14 alek lidtke
 *                           started using strcpy_s in line 1207 with MSVS compiler
 *                           started using fopen_s in lines 1219, 1263, 1452, 1462, 1470, 1524, and 1530 with MSVS compiler
 *                           added explicit casts from double to int in lines 1044, 1045, 1046, 1047, 1091, 1438, and 1441 to avoid possible loss of data
 *     changes :
...
//  MAKE SURE TO ROUND-OFF ONLY DOWN, ALSO FOR NEGATIVE MJD:
      JDAY = MJDay + 18262.0;
      L = (int) floor((4000.0 * (JDAY + 18204))/1461001);
      N = (int) JDAY - (int) floor((1461.0 * L)/4) + 18234;
      M = (int) floor((80.0 * N)/2447);
      day = N - (int) floor((2447.0 * M)/80);
      JJ  = (int)floor(M/11.0);
      mon  = M + 2 - 12*JJ;
      year = 1900 + L + JJ;
     ...
     )
{
     ...
     L   = year - (int) JJ - 1900 * (int)floor(year/1900.0) + 100 * (int)floor(2000.0/(year + 1951));
    ...
    )
{
	  ...
	  max_lines = 3000;
#ifdef _MSC_VER
	  strcpy_s( path_to_a_file,100,"");
	  strcpy_s( path_to_a_file,100,"d:/Codes/DTM/dtmsrc/data/am_file_spiderx.dat");  // geomagnetic indices
	  strcpy_s( path_to_f_file,100,"");
	  strcpy_s( path_to_f_file,100,"d:/Codes/DTM/dtmsrc/data/proxies_unadjustedx.dat");  // solar flux values
#else
      ...
      file_io_status = 0;
#ifdef _MSC_VER
	  errno_t fileStatus = fopen_s( &infile, path_to_f_file, "r");
#else
...
#ifdef _MSC_VER
		  errno_t fileStatus2  = fopen_s(&infile, path_to_a_file, "r");
#else
          ...
          while (feof(infile) == 0)
  		  {
			  ...
			  // substrings
			  sscanf_s(longstr1,"%20c %4i %4i %4i %4i %4i %4i %4i %4i %4i ", &temp20,
							     ...
							     &a_indexes[idx].am[8]);
              strncpy_s(temp4,&longstr1[0],4);  // get substring value
              strncpy_s(temp4a,&longstr1[4],4);
              strncpy_s(temp4b,&longstr1[8],4);
...
{ 
  ...
  // --------------------------------------------------
    strcpy_s( infilename, PATH_TO_DEN_UNC_1);
#ifdef _MSC_VER
	errno_t inFile1Status = fopen_s(&infile1, infilename, "r");  // files with all 432 values on 1 line
#else
	...
	strcpy_s(infilename, PATH_TO_DEN_UNC_2);
#ifdef _MSC_VER
	errno_t infile2status = fopen_s(&infile2,infilename, "r");
#else
	...
	strcpy_s(infilename, PATH_TO_DEN_UNC_3);
#ifdef _MSC_VER
	errno_t infile3status = fopen_s(&infile3, infilename, "r");
#else
...
//		sscanf_s(longstr1,"%{%g} ", &alt_scale );
    ...
    fclose (infile3);
#ifdef _MSC_VER
	errno_t DTM12inFileStatus  = fopen_s(&infile, PATH_TO_DTM2012_IN_FILE, "r");
#else
	...
	if (IVERIF == 1){
#ifdef _MSC_VER
		errno_t outfileStatus = fopen_s( &outfile, PATH_TO_DTM2012_OUT_FILE, "w");
#else
