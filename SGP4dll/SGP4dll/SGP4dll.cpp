// SGP4dll.cpp : Defines the exported functions for the DLL application.
//
...
*                         changed a number of SGP4name to SGP4Funcs namespace idenitifier
*                         started using sscanf_s in lines 2277, 2290, 2305, 2320, and 2334
*                         started using scanf_s in lines 2391, 2401, 2412, 2423, 2445, 2455
*                         started using strcpy_s in lines 2879, 2889, 2911
*    changes :
...
namespace SGP4Funcs{
	...
	*    vallado, crawford, hujsak, kelso  2006
		...
		)
	{
		...
		if (opsmode == 'a')
		{
			...
			gsto = SGP4Funcs::gstime(epoch + 2433281.5);
	...
	*    vallado, crawford, hujsak, kelso  2006
		...
		)
	{
		...
		SGP4Funcs::getgravconst(whichconst, satrec.tumin, satrec.mu, satrec.radiusearthkm, satrec.xke,
		...
		initl
			...
			posq, rp, rteosq, sinio, satrec.gsto, satrec.operationmode, satrec.no);
	...
	*    vallado, crawford, hujsak, kelso  2006
		...
		)
	{
		...
		SGP4Funcs::jday(year, mon, day, hr, minute, sec, satrec.jdsatepoch);
		...
		if ((typerun != 'v') && (typerun != 'c'))
		{
...
#endif
				fflush(stdin);
				SGP4Funcs::jday(startyear, startmon, startday, starthr, startmin, startsec, jdstart);
...
#endif
				fflush(stdin);
				SGP4Funcs::jday(stopyear, stopmon, stopday, stophr, stopmin, stopsec, jdstop);
			...
			if (typeinput == 'd')
			{
				...
				SGP4Funcs::days2mdhms(startyear, startdayofyr, mon, day, hr, minute, sec);
				SGP4Funcs::jday(startyear, mon, day, hr, minute, sec, jdstart);
				SGP4Funcs::days2mdhms(stopyear, stopdayofyr, mon, day, hr, minute, sec);
				SGP4Funcs::jday(stopyear, mon, day, hr, minute, sec, jdstop);
