...
*
*    current :
*9 aug 14 alek lidkte
*                          changed constructors of std::vector< std::vector<double> > in lines 114, 817, 821, 822, 823, 824, 1299, 1300, 1301, 1302, 1303, 1304, 1305, 1306, 1307, 1308, 1309, and 1310 to match C++ 11 standard.
*    changes :
...
)
{
	...
	if (direct == eTo)
	{
		...
		case 'v':
		{
					...
					vnom[1] = xnom[4];
					vnom[2] = xnom[5];
					ast2Body::rv2coe(rnom, vnom, p, a, ecc, incl, omega, argp, nu, m, arglat, truelon, lonper);
...
)
{
	...
	SGP4Funcs::sgp4init(whichconst, satrec.operationmode, satrec.satnum, satrec.jdsatepoch - 2433281.5, satrec.bstar,
		satrec.xndot, satrec.xnddot, satrec.ecco, satrec.argpo, satrec.inclo, satrec.mo, satrec.no,
		satrec.nodeo, satrec);
